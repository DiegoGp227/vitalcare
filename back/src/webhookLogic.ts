...
// ❌ Eliminamos la importación de OpenAI
// import OpenAI from "openai"; 
import { Request, Response } from "express"; 

// 🆕 Importamos el nuevo cliente de Google Gen AI
...
// Por defecto, leerá GOOGLE_API_KEY del entorno, al igual que lo hacía OpenAI
// Si quieres usar una variable de entorno diferente (por ejemplo, GOOGLE_GEMINI_KEY), 
// puedes pasarla explícitamente:
const GEMINI_API_KEY = "AIzaSyAIXyWwQ217sz4jMHKk2HiDnevfwws8Vms"; 

// 🆕 Inicializamos el cliente de Gemini
// Usa el modelo que nos indicaste: gemini-2.5-flash
const GEMINI_MODEL = "gemini-2.5-flash";

export const ai = new GoogleGenAI({ 
    apiKey: GEMINI_API_KEY, // Opcional si ya está en GOOGLE_API_KEY
});

// ❌ Eliminamos la exportación de openai
// export const openai = new OpenAI({ 
//     apiKey: process.env.OPENAI_API_KEY,
// });

// =======================================================
// === 🌐 CONFIGURACIÓN HASURA (Sin cambios)
// =======================================================
...
// =======================================================
// === 💾 ESTADO DE LA CONVERSACIÓN (Sin cambios)
// =======================================================
/**
...
// =======================================================
// === 📞 FUNCIONES AUXILIARES Y LÓGICA HASURA (Sin cambios)
// =======================================================
...
// Enviar mensaje a WhatsApp (Sin cambios)
...
/**
 * Procesa el número de cédula, llama a la API de Hasura y establece el siguiente estado. (Sin cambios)
 */
...
// ------------------------------------
// 🆕 FUNCIÓN 1: EXTRAER SÍNTOMAS CON LLM (MIGRADO A GEMINI)
// ------------------------------------
...
/**
 * Usa Gemini para extraer el síntoma principal y su zona de un texto libre.
 */
async function extractSymptomDetails(symptomsText: string): Promise<SymptomDetails> {
    try {
        // 🆕 Definición del esquema JSON para la respuesta (Schema)
        const responseSchema = {
            type: Type.OBJECT,
            properties: {
                dolor: {
                    type: Type.STRING,
                    description: "Descripción concisa del síntoma principal (ej: Fiebre, Dolor de cabeza). Si no hay síntoma específico, usar 'Malestar general/No especificado'."
                },
                zonadolor: {
                    type: Type.STRING,
                    description: "Ubicación del dolor (ej: Cabeza, Abdomen, N/A). Usar 'N/A' si no aplica."
                }
            },
            required: ["dolor", "zonadolor"]
        };
        ...
        // 🆕 Llamada a Gemini con forzado de respuesta JSON (responseMimeType y responseSchema)
        const aiResponse = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: responseSchema, // Usamos el esquema definido
                temperature: 0.1,
                systemInstruction: "Eres un extractor de datos de síntomas médicos. Tu única tarea es devolver un objeto JSON válido según las instrucciones, usando las claves 'dolor' y 'zonadolor'."
            }
        });

        // 🆕 La respuesta JSON está en aiResponse.text
        ...
        // Log para ver el error de la librería o de la llamada
        console.error("❌ Error en la extracción de síntomas con Gemini:", error); 
        return { dolor: 'Error de extracción', zonadolor: 'Error de extracción' };
    }
}

// ------------------------------------
// 🆕 FUNCIÓN 2: GUARDAR EN HASURA (Sin cambios)
// ------------------------------------
...
// =======================================================
// === 💬 HANDLER PRINCIPAL DEL WEBHOOK (ACTUALIZADO CON GEMINI)
// =======================================================
...
export async function processWebhookMessage(req: Request, res: Response) {
    try {
        ...
        // --- LÓGICA DE ESTADO (ASKING_CEDULA y lógica Hasura sin cambios) ---
        ...
        if (currentState === 'START') {
            ...
            if (!pacienteid) {
                ...
                // 2. Extraer detalles con Gemini (función actualizada)
            ...
            // 🆕 Llamada a Gemini para conversación simple
            const aiResponse = await ai.models.generateContent({
                model: GEMINI_MODEL,
                contents: [
                ...
                config: {
                    temperature: 0.7,
                    systemInstruction: "Eres Paulo, un asistente. El usuario ya se registró en recepción. Responde a sus consultas de forma útil."
                }
            });

            // 🆕 La respuesta de texto está en aiResponse.text
            const reply: string | any = aiResponse.text;
